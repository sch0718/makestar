...
import com.makestar.chat.dto.AddParticipantsRequest;
import com.makestar.chat.dto.ChatRoomDto;
import com.makestar.chat.dto.CreateChatRoomRequest;
import com.makestar.chat.model.ChatRoom;
import com.makestar.chat.service.ChatRoomService;
...
public class ChatRoomController {
     ...
     * 
     * @param type 채팅방 유형 (DIRECT 또는 GROUP)
     * @param participantIds 참여자 ID 목록 (쉼표로 구분)
     * @return 조건에 맞는 채팅방 목록
     */
    @GetMapping("/search")
    public ResponseEntity<List<ChatRoomDto>> findByTypeAndParticipants(
            @RequestParam("type") String type,
            @RequestParam("participants") String participants) {
        log.info("Searching chat rooms by type: {} and participants: {}", type, participants);
        ...
        List<ChatRoomDto> chatRooms = chatRoomService.findByTypeAndParticipantIdsContainingAll(roomType, participantList);
        return ResponseEntity.ok(chatRooms);
    }
