...
import com.makestar.chat.repository.ChatRoomRepository;
...
public class ChatRoomServiceImpl implements ChatRoomService {
     ...
     */
    @Override
    public List<ChatRoomDto> findByTypeAndParticipantIdsContainingAll(ChatRoom.ChatRoomType type, List<String> participantIds) {
        log.info("Finding chat rooms by type: {} and participants: {}", type, participantIds);
        
        List<ChatRoom> chatRooms = chatRoomRepository.findByTypeAndParticipantIdsContainingAll(
            type, 
            participantIds,
            participantIds.size()
        );
        ...
        return chatRooms.stream()
                .map(chatRoom -> {
                    ChatMessageDto lastMessage = chatMessageService.getLatestMessage(chatRoom.getId());
                    // 읽지 않은 메시지 수는 0으로 설정 (필요한 경우 나중에 구현)
                    return ChatRoomDto.fromEntityWithLastMessage(chatRoom, lastMessage, 0);
                })
                .collect(Collectors.toList());
    }
