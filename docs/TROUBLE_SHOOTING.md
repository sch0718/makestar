...
DB 설계 얘기가 나온 김에 현재 만들어진 테이블들을 살펴보면 내가 직접 했다면 이렇게 하진 않았을 것 같다.
...
![API 문서 - 인증](swagger_auth.png)

![API 문서 - 채팅](swagger_chat.png)
...
다시 이것저것 보안과 인증 관련한 코드들을 수정하고, 웹앱 코드도 수정했다.

![로그인 화면](login_screenshot.png)

![회원 가입 화면](register_screenshot.png)
